...
function displayHealthInitiatives(product) {
    ...
    if (product.nutriments) {
        ...
        if (fiber >= 3) {
            ...
            initiatives.push({
                icon: 'fa-carrot',
                text: 'Low in fiber. Add fruits and vegetables to increase fiber intake.',
                type: 'info'
            });
        }
        ...
        if (protein >= 10) {
            initiatives.push({
                icon: 'fa-dumbbell',
                text: 'Good protein content! Supports muscle health and satiety.',
                type: 'success'
            });
        }
        ...
        if (sugar > 22.5) {
            initiatives.push({
                icon: 'fa-candy-cane',
                text: 'High sugar content. Limit consumption to maintain healthy blood sugar.',
                type: 'warning'
            });
        }
        ...
        if (salt > 1.5) {
            initiatives.push({
                icon: 'fa-tint',
                text: 'High salt content. Excessive salt can raise blood pressure.',
                type: 'warning'
            });
        }
    }
    ...
    initiatives.push({
        icon: 'fa-utensils',
        text: 'Read labels carefully and choose products with minimal processing.',
        type: 'info'
    });
    
    initiatives.push({
        icon: 'fa-shopping-basket',
        text: 'Prefer local and seasonal products when possible for better freshness.',
        type: 'info'
    });
    
    // Display initiatives (limit to 5 most relevant)
    const displayInitiatives = initiatives.slice(0, 5);
    
    if (displayInitiatives.length > 0) {
        healthInitiativesList.innerHTML = displayInitiatives.map(init => {
            const typeClass = init.type === 'success' ? 'health-tip-success' : 
                            init.type === 'warning' ? 'health-tip-warning' : 'health-tip-info';
            return `
                <div class="health-tip ${typeClass}">
